      ...
      <div className="w-full max-w-md">
        ...
        {status === 'success' && (
          <div className="card-dark p-8 text-center space-y-6">
            ...
            <div>
              <h1 className="text-2xl font-black text-white mb-2">Bem-vindo ao Premium! 🎉</h1>
              <p className="text-dark-400 text-sm">Seu pagamento foi confirmado. O plano Premium já está ativo.</p>
            </div>
            {paymentId && (
            ...
            <div className="grid grid-cols-2 gap-3 text-left">
              {[
                ...
                <div key={f.label} className="flex items-center gap-2 text-sm text-dark-300">
                  <span className="text-brand-400 font-bold">{f.icon}</span> {f.label}
                </div>
              ))}
            </div>
            <button onClick={() => router.push('/dashboard')} className="btn-primary w-full justify-center py-3 text-base">
              <Sparkles className="w-5 h-5" /> Acessar meu dashboard <ArrowRight className="w-5 h-5" />
            </button>
          </div>
        )}
        ...
        {status === 'pending' && (
          <div className="card-dark p-8 text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-dark-400 text-sm">Seu pagamento está sendo processado. Assim que aprovado, seu acesso Premium será liberado.</p>
            </div>
            {paymentId && (
              <div className="bg-dark-800 rounded-xl px-4 py-3 text-left">
                <p className="text-xs text-dark-500 mb-1">ID do pagamento</p>
        ...
        {status === 'error' && (
          <div className="card-dark p-8 text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-dark-400 text-sm">Não conseguimos confirmar seu pagamento. Se já pagou, entre em contato pelo WhatsApp com o comprovante.</p>
            </div>
            <div className="flex flex-col gap-3">
              <a href="https://mpago.la/2jcbcsh" target="_blank" rel="noopener noreferrer" className="btn-primary w-full justify-center py-3">
